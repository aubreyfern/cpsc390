...
Euclidean::Euclidean(char** array, int r){
  theArray=array;
  rows=r;
  findInitial();
  findGoal();
...
//euclidean formula
float Euclidean::euclidean(int x, int y){
  int curr_x = x;
  int curr_y = y;
...
//manhattan formula
float Euclidean::manhattan(int x, int y){
  int curr_x = x;
  int curr_y = y;
...
void Euclidean::simEuclidean(){
  ...
  if(y-1>=0)
  {
    ...
    if(isOpen(theArray[x][y-1])){
      float leftdist = euclidean(x,y-1);
  ...
  {
    if(isOpen(theArray[x+1][y])){
      float down_dist = euclidean(x+1,y);
      if(!set)
      {
...
bool Euclidean::isOpen(char position){
  if(position=='.'){
    return true;
  }
  else if(position=='g'){
...
//print to file
string Euclidean::PrintFile()
{
	stringstream write;
  write<<"EUCLIDEAN DISTANCE:"<<endl;
